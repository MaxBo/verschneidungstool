...
def startmain():
    ...
    parser.add_argument("--config", action="store",
                        help="Pfad zu einer XML-Config-Datei mit den "
                        "Verbindungsdaten und Pfaden zu den Postgres-Tools "
                        "(optional)",
                        dest="config")
    ...
    parser.add_argument("--dpfad", action="store",
                        help="Dateipfad für Ergebnisdownload, Dateiendung "
    ...
    parser.add_argument("--name", action="store",
                        help="Name der Tabelle mit den Shapes (optional bei "
                        "Shapefile-Upload, Pflicht bei Verschneidung)",
                        dest="table_name")
    ...
    if arguments.upload and (not arguments.shape_path or
                             not arguments.schema):
        print('Um ein Shapefile hochzuladen, müssen Pfad zum Shapefile '
              'und Zielschema angegeben sein.')
        exit(1)

    if arguments.intersection and (not arguments.table_name or
                                   not arguments.schema):
        print('Um eine Verschneidung durchzuführen hochzuladen, müssen '
              'Schema und Tabellenname mit den Shapes angegeben sein.')
        exit(1)
